...
describe('a line stored before the phone wrote down the name of its station', () => {
  beforeEach(() => {
    setActivePinia(createPinia())
    localStorage.clear()
  })

  function anOlderDraftRoutedTo(stationId: string): void {
    localStorage.setItem(
      DRAFT_STORAGE_KEY,
      JSON.stringify({
        tableName: 'Tisch 3',
        note: null,
        lines: [{ catalogItemId: 'item-wasser', note: null, stationId, name: 'Wasser' }],
        clientOrderId: 'order-1',
      }),
    )
  }

  function theBarsThatArePouring(): void {
    const catalog = useCatalogStore()
    catalog.catalog = {
      categories: [],
      items: [
  ...
  it('takes the name from the item list as soon as the phone has been given it', async () => {
    anOlderDraftRoutedTo('station-theke-innen')
    const order = useOrderStore()

    theBarsThatArePouring()
    await nextTick()

    expect(order.draft.lines[0].stationName).toBe('Theke innen')
  })

  it('keeps that name, so a later reload still reads it', async () => {
    anOlderDraftRoutedTo('station-theke-innen')
    useOrderStore()
  ...
  it('stays unnamed when the item list no longer carries that station either', async () => {
    anOlderDraftRoutedTo('station-abgebaut')
    const order = useOrderStore()

    theBarsThatArePouring()
    await nextTick()

    expect(order.draft.lines[0].stationName).toBe('')
  })
})
