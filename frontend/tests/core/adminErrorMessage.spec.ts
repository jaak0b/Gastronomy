import { describe, expect, it } from 'vitest'
import { adminBlockingConditions, adminErrorMessage } from '../../src/core/adminErrorMessage'

describe('adminErrorMessage, the key the laptop actually sent', () => {
  it('renders an open-slips refusal as an open-slips message', () => {
    const message = adminErrorMessage({
      code: 'Conflict',
      messageKey: 'admin.locations.openTickets',
      parameters: { count: 3 },
      details: null,
    })

    expect(message).toEqual({
      key: 'admin.locations.openTickets',
      parameters: { count: 3 },
      count: 3,
    })
  })

  it('renders an orphaned-items refusal as its own message, never as open slips', () => {
    const message = adminErrorMessage({
      code: 'Conflict',
      messageKey: 'admin.itemsWouldHaveNoStation',
      parameters: { count: 2 },
      details: null,
    })

    expect(message.key).toBe('admin.itemsWouldHaveNoStation')
  })

  it('renders the missing printer connection as its own message', () => {
    const message = adminErrorMessage({
      code: 'Conflict',
      messageKey: 'admin.stationHasNoPrinterWorker',
      parameters: {},
      details: null,
    })

    expect(message.key).toBe('admin.stationHasNoPrinterWorker')
  })

  it('carries the count so the sentence can take its singular form', () => {
    const message = adminErrorMessage({
      code: 'Conflict',
      messageKey: 'admin.itemsWouldHaveNoStation',
      parameters: { count: 1 },
      details: null,
    })

    expect(message.count).toBe(1)
  })

  it('carries no count for a message that has no number in it', () => {
    const message = adminErrorMessage({
      code: 'Conflict',
      messageKey: 'admin.stationHasNoPrinterWorker',
      parameters: {},
      details: null,
    })

    expect(message.count).toBeNull()
  })
})

describe('adminErrorMessage, a key this app does not know', () => {
  it('falls back to the general message rather than mislabelling the refusal', () => {
    const message = adminErrorMessage({
      code: 'Conflict',
      messageKey: 'admin.somethingAddedLater',
      parameters: { count: 4 },
      details: null,
    })

    expect(message.key).toBe('admin.loadFailed')
  })

  it('drops the parameters of a key it cannot render', () => {
    const message = adminErrorMessage({
      code: 'Conflict',
      messageKey: 'admin.somethingAddedLater',
      parameters: { count: 4 },
      details: null,
    })

    expect(message.parameters).toEqual({})
  })

  it('falls back when the laptop sent no error body at all', () => {
    const message = adminErrorMessage(null)

    expect(message).toEqual({ key: 'admin.loadFailed', parameters: {}, count: null })
  })
})

describe('adminBlockingConditions, why a new event cannot start yet', () => {
  it('renders every condition the laptop listed', () => {
    const conditions = adminBlockingConditions({
      blockingConditions: [
        { guard: 'OpenTickets', messageKey: 'admin.event.blockedOpenTickets', parameters: { count: 4 } },
        { guard: 'OpenQuestions', messageKey: 'admin.event.blockedQuestions', parameters: { count: 2 } },
      ],
    })

    expect(conditions.map((condition) => condition.key)).toEqual([
      'admin.event.blockedOpenTickets',
      'admin.event.blockedQuestions',
    ])
  })

  it('keeps each condition its own count, so each sentence takes its own form', () => {
    const conditions = adminBlockingConditions({
      blockingConditions: [
        { guard: 'OpenTickets', messageKey: 'admin.event.blockedOpenTickets', parameters: { count: 4 } },
        { guard: 'OpenQuestions', messageKey: 'admin.event.blockedQuestions', parameters: { count: 1 } },
      ],
    })

    expect(conditions.map((condition) => condition.count)).toEqual([4, 1])
  })

  it('renders a condition that names stations rather than a count', () => {
    const conditions = adminBlockingConditions({
      blockingConditions: [
        { guard: 'MockTransport', messageKey: 'admin.event.blockedMock', parameters: { names: 'Küche' } },
      ],
    })

    expect(conditions[0]).toEqual({
      key: 'admin.event.blockedMock',
      parameters: { names: 'Küche' },
      count: null,
    })
  })

  it('reads a bare list, because the laptop may answer without an envelope', () => {
    const conditions = adminBlockingConditions([
      { guard: 'OpenTickets', messageKey: 'admin.event.blockedOpenTickets', parameters: { count: 4 } },
    ])

    expect(conditions).toHaveLength(1)
  })

  it('falls back to the general message for a guard this app does not know', () => {
    const conditions = adminBlockingConditions({
      blockingConditions: [{ guard: 'Added later', messageKey: 'admin.somethingNew', parameters: {} }],
    })

    expect(conditions[0].key).toBe('admin.loadFailed')
  })

  it('says the general message once rather than repeating it per unknown guard', () => {
    const conditions = adminBlockingConditions({
      blockingConditions: [
        { guard: 'One', messageKey: 'admin.somethingNew', parameters: {} },
        { guard: 'Two', messageKey: 'admin.somethingElse', parameters: {} },
      ],
    })

    expect(conditions).toHaveLength(1)
  })

  it('lists nothing when the laptop sent nothing to list', () => {
    expect(adminBlockingConditions(null)).toEqual([])
  })

  it('lists nothing for a payload that carries no conditions', () => {
    expect(adminBlockingConditions({ code: 'Conflict' })).toEqual([])
  })
})
