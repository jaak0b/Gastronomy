...
description: Use when adding or changing a Playwright webtest in e2e/**/*.spec.ts for the order placement flow, order routing to production locations, printing and print-failure handling, device enrolment by QR, or admin configuration, or when a core logic change needs a test that would actually catch it, before committing the change.
---
...
`e2e/<feature>/<name>.flow.md`, committed alongside the test, so it is reviewable and survives
...
(`e2e/<feature>/golden/`) holding that feature's validated scan sets, gcode goldens, and one
...
synthetic render ever lives under `e2e/`: a synthetic image cannot be physically validated, so
...
1. **One webtest per case, and every webtest is a full user journey.** Start at the app's entry
   ...
   `page.addInitScript` into localStorage (it lives at `e2e/em.spec.ts` today and moves to
   `e2e/flow/em.spec.ts` after the reorganization). Going forward, such a shortcut must carry a comment
...
2. **Real scans only under `e2e/`; synthetic renders stay in the unit tree.** A real flatbed
   ...
   belong to the unit test tree (`tests/`) as render-recovery, never as a webtest fixture and
   never under `e2e/`.
...
- **Track every real scan and gcode golden with Git LFS.** `.gitattributes` should cover
  `e2e/**/*.png` and the gcode golden extension so the feature-local `golden/` binaries and
  gcode files do not leak into plain git history.
- **Upload and wait pattern**, mirrored from the EM webtest (`e2e/em.spec.ts` today, moving to
  `e2e/flow/em.spec.ts` after the reorganization):
  ```ts
...
- **Run with `npm run e2e`** from `frontend/`. A new webtest must pass locally before it is
