...
public sealed class CatalogRepositoryTest
{
  ...
  public async Task ReadAtFestivalAsync_TheSeededFestival_CarriesItsStationsCategoriesAndPricedItems()
  {
    ...
    CatalogAtFestival catalog = await repository.ReadAtFestivalAsync(seeded.FestivalId,
                                                                     "Sommerfest",
                                                                     TestContext.CurrentContext.CancellationToken);

    CatalogItemRow sausage = catalog.Items.Single(item => item.ItemId == seeded.SausageItemId);
  ...
  public async Task ReadAtFestivalAsync_AnItemWhoseCategoryIsSwitchedOff_LeavesTheItemOut()
  {
    ...
    var drinks = await fixture.DbContext.CatalogCategories
                              .FirstAsync(category => category.Id == seeded.DrinkCategoryId,
                                          TestContext.CurrentContext.CancellationToken);
