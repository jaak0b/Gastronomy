...
public sealed class StationQueueWriterTest
{
  ...
  public void SetUp()
  {
    ...
    A.CallTo(() => _repository.FindItemsAtStationAsync(A<IReadOnlyCollection<Guid>>._,
                                                        A<Guid>._,
  ...
  public async Task FulfillAsync_AnOpenItemAtThisStation_StampsItAndCommits()
  {
    ...
    Result<FulfillmentResult, StationQueueFailure> written =
      await _writer.FulfillAsync([bratwurst.Id], _stationId, TestContext.CurrentContext.CancellationToken);
  ...
  {
    Result<FulfillmentResult, StationQueueFailure> written =
      await _writer.FulfillAsync([Guid.NewGuid()], _stationId, TestContext.CurrentContext.CancellationToken);
  ...
  {
    Result<FulfillmentResult, StationQueueFailure> written =
      await _writer.FulfillAsync([], _stationId, TestContext.CurrentContext.CancellationToken);
  ...
  {
    Assert.That(async () => await _writer.FulfillAsync(null!,
                                                        _stationId,
  ...
  public async Task UnfulfillAsync_AnItemTheStationHandedOut_ClearsTheStampAndCommits()
  {
    ...
    Result<FulfillmentResult, StationQueueFailure> written =
      await _writer.UnfulfillAsync([bratwurst.Id], _stationId, TestContext.CurrentContext.CancellationToken);
  ...
  {
    Assert.That(async () => await _writer.UnfulfillAsync(null!,
                                                          _stationId,
                                                          TestContext.CurrentContext.CancellationToken),
                Throws.ArgumentNullException);
  }
  ...
  {
    StationOrder stationOrder = StationOrderWith(DeliveryMode.AsItComes);
    GivenStationOrder(stationOrder);

    Result<StationOrder, StationQueueFailure> hidden =
      await _writer.HideFromAsItComesQueueAsync(stationOrder.Id,
                                                 _stationId,
                                                 _festivalId,
                                                 TestContext.CurrentContext.CancellationToken);
  ...
  {
    StationOrder stationOrder = StationOrderWith(DeliveryMode.Together);
    GivenStationOrder(stationOrder);
  ...
  {
    A.CallTo(() => _repository.FindAtStationAsync(A<Guid>._, A<Guid>._, A<Guid>._, A<CancellationToken>._))
     .Returns(Task.FromResult<StationOrder?>(null));

    Result<StationOrder, StationQueueFailure> hidden =
      await _writer.HideFromAsItComesQueueAsync(Guid.NewGuid(),
                                                 _stationId,
                                                 _festivalId,
                                                 TestContext.CurrentContext.CancellationToken);
    ...
    Assert.Multiple(() =>
                    {
                      ...
                      Assert.That(hidden.Failure.Reason,
                                  Is.EqualTo(StationQueueFailureReason.OrderNotAtThisStation));
                    });
  }
  ...
  {
    A.CallTo(() => _repository.FindAtStationAsync(stationOrder.Id,
                                                   _stationId,
                                                   _festivalId,
                                                   A<CancellationToken>._))
