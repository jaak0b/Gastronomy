...
public sealed class FestivalAdministrationServiceTest
{
  ...
  public void SetUp()
  {
    ...
    A.CallTo(() => _repository.FindAllAsync(A<CancellationToken>._))
     .Returns(Task.FromResult<IReadOnlyCollection<Festival>>([]));
    A.CallTo(() => _repository.FindContentCountsAsync(A<CancellationToken>._))
     .Returns(Task.FromResult<IReadOnlyList<FestivalContentCounts>>([]));
  ...
  {
    Assert.That(async () => await _service.UpdateAsync(_festivalId, null!, CancellationToken.None),
                Throws.ArgumentNullException);
  }
  ...
  {
    Assert.That(async () => await _service.CopyAsync(_festivalId, null!, CancellationToken.None),
                Throws.ArgumentNullException);
  }
  ...
  {
    A.CallTo(() => _repository.FindAllAsync(A<CancellationToken>._))
     .Returns(Task.FromResult<IReadOnlyCollection<Festival>>([BuildFestival(_festivalId, false)]));
  ...
  public async Task ListAsync_AFestivalWithStationsItemsAndOrders_CarriesItsOwnCounts()
  {
    A.CallTo(() => _repository.FindAllAsync(A<CancellationToken>._))
     .Returns(Task.FromResult<IReadOnlyCollection<Festival>>([BuildFestival(_festivalId, false)]));
    A.CallTo(() => _repository.FindContentCountsAsync(A<CancellationToken>._))
     .Returns(Task.FromResult<IReadOnlyList<FestivalContentCounts>>([new(_festivalId, 2, 7, 13)]));
  ...
  public async Task CreateAsync_ANameOfOnlySpaces_FailsBecauseTheNameIsMissing()
  {
    Result<SavedFestival, FestivalAdministrationFailure> created =
      await _service.CreateAsync(BuildPeriodRequest("   ", _now, _now.AddHours(6)), CancellationToken.None);

    Assert.Multiple(() =>
                    {
                      Assert.That(created.IsSuccess, Is.False);
                      Assert.That(created.Failure.Reason,
                                  Is.EqualTo(FestivalAdministrationFailureReason.NameMissing));
  ...
  {
    Result<SavedFestival, FestivalAdministrationFailure> created =
      await _service.CreateAsync(BuildPeriodRequest("Sommerfest", _now, _now), CancellationToken.None);

    Assert.Multiple(() =>
                    {
                      Assert.That(created.IsSuccess, Is.False);
                      Assert.That(created.Failure.Reason,
                                  Is.EqualTo(FestivalAdministrationFailureReason.PeriodInvalid));
                    });
  }
  ...
  public async Task CreateAsync_APeriodAnotherFestivalAlreadyCovers_NamesTheFestivalInTheWay()
  {
    ...
    Result<SavedFestival, FestivalAdministrationFailure> created =
      await _service.CreateAsync(BuildPeriodRequest("Herbstfest", _now, _now.AddHours(2)),
    ...
    Assert.Multiple(() =>
                    {
                      Assert.That(created.IsSuccess, Is.False);
                      Assert.That(created.Failure.Reason,
                                  Is.EqualTo(FestivalAdministrationFailureReason.PeriodOverlapsAnotherFestival));
  ...
  {
    Result<SavedFestival, FestivalAdministrationFailure> created =
      await _service.CreateAsync(BuildPeriodRequest("Sommerfest", _now, _now.AddHours(6)),
                                 CancellationToken.None);
    ...
    A.CallTo(() => _repository.AddAsync(A<Festival>.That.Matches(festival => festival.Name == "Sommerfest"),
  ...
  public async Task CopyAsync_AFestivalThatIsNotThere_FailsBecauseTheFestivalIsNotFound()
  {
    ...
    Result<SavedFestival, FestivalAdministrationFailure> copied =
      await _service.CopyAsync(_festivalId,
                               BuildPeriodRequest("Sommerfest 2027", _now, _now.AddHours(6)),
                               CancellationToken.None);

    Assert.Multiple(() =>
                    {
                      Assert.That(copied.IsSuccess, Is.False);
                      Assert.That(copied.Failure.Reason,
                                  Is.EqualTo(FestivalAdministrationFailureReason.FestivalNotFound));
                    });
  }
  ...
  public async Task CopyAsync_AFestivalThatIsThere_CopiesItsStationsItemsAndAssignmentsAcross()
  {
    ...
    A.CallTo(() => _repository.CopyContentsAsync(_festivalId, copied.Value.FestivalId, A<CancellationToken>._))
     .MustHaveHappenedOnceExactly();
  }
  ...
  {
    A.CallTo(() => _repository.FindByIdAsync(_festivalId, A<CancellationToken>._))
     .Returns(Task.FromResult<Festival?>(BuildFestival(_festivalId, false)));
    ...
    Assert.Multiple(() =>
                    {
                      ...
                      Assert.That(hidden.Failure.Reason,
                                  Is.EqualTo(FestivalAdministrationFailureReason.FestivalIsRunning));
  ...
  {
    A.CallTo(() => _repository.FindByIdAsync(_festivalId, A<CancellationToken>._))
     .Returns(Task.FromResult<Festival?>(BuildFestival(_festivalId, true)));
  ...
  {
    Festival festival = BuildFestival(_festivalId, true);
    A.CallTo(() => _repository.FindByIdAsync(_festivalId, A<CancellationToken>._))
     .Returns(Task.FromResult<Festival?>(festival));

    Result<SavedFestival, FestivalAdministrationFailure> shown =
      await _service.ShowAsync(_festivalId, CancellationToken.None);
  ...
  public async Task UpdateAsync_AFestivalThatIsNotThere_FailsBecauseTheFestivalIsNotFound()
  {
    Result<SavedFestival, FestivalAdministrationFailure> updated =
      await _service.UpdateAsync(_festivalId,
                                 BuildPeriodRequest("Sommerfest", _now, _now.AddHours(6)),
                                 CancellationToken.None);

    Assert.Multiple(() =>
                    {
                      Assert.That(updated.IsSuccess, Is.False);
                      Assert.That(updated.Failure.Reason,
                                  Is.EqualTo(FestivalAdministrationFailureReason.FestivalNotFound));
                    });
  }
