...
public sealed class RetryPolicyTest
{
  ...
  private void AssertMapping(PrintOutcomeMapping mapping,
  ...
  {
    AssertMapping(_retryPolicy.Map(PrintOutcome.Confirmed, 512),
  ...
  {
    AssertMapping(_retryPolicy.Map(PrintOutcome.Blocked, 0),
  ...
  {
    AssertMapping(_retryPolicy.Map(PrintOutcome.Unreachable, 0),
  ...
  {
    AssertMapping(_retryPolicy.Map(PrintOutcome.SocketDropped, 0),
                  PrintJobStatus.Queued,
                  PrintJobStatus.Queued,
                  true);
  }
  ...
  {
    AssertMapping(_retryPolicy.Map(PrintOutcome.SocketDropped, 1),
  ...
  {
    AssertMapping(_retryPolicy.Map(PrintOutcome.Timeout, 0),
                  PrintJobStatus.Queued,
                  PrintJobStatus.Queued,
                  true);
  }
  ...
  {
    AssertMapping(_retryPolicy.Map(PrintOutcome.Timeout, 1),
                  PrintJobStatus.Unknown,
                  PrintJobStatus.Unknown,
                  false);
  }
  ...
  {
    AssertMapping(_retryPolicy.Map(PrintOutcome.PrinterError, 0),
                  PrintJobStatus.Blocked,
                  PrintJobStatus.Blocked,
                  true);
  }
  ...
  {
    AssertMapping(_retryPolicy.Map(PrintOutcome.PrinterError, 1),
                  PrintJobStatus.Unknown,
                  PrintJobStatus.Unknown,
                  false);
  }
  ...
  {
    Assert.Throws<InvalidOperationException>(() => _retryPolicy.Map(PrintOutcome.Confirmed, 0));
  }
  ...
  public void Map_AZeroByteOnlyOutcomeReportedWithBytesWritten_Throws()
  {
    Assert.Multiple(() =>
                    {
                      Assert.Throws<InvalidOperationException>(() => _retryPolicy.Map(PrintOutcome.Blocked, 1));
                      Assert.Throws<InvalidOperationException>(() => _retryPolicy.Map(PrintOutcome.Unreachable, 1));
                    });
  }
  ...
  public void Map_TheOutcomeTableItself_CoversEveryDeclaredPrintAttemptOutcome()
  {
    Assert.Multiple(() =>
                    {
                      Assert.That(OutcomesTheTableDefinesWithBytesWritten().Keys,
                                  Is.EquivalentTo(Enum.GetValues<PrintOutcome>()));
                      Assert.That(OutcomesTheTableDefinesWithNoBytesWritten().Keys,
                                  Is.EquivalentTo(Enum.GetValues<PrintOutcome>()));
                    });
  }
  ...
  public void Map_EveryOutcomeAndByteCount_EitherMapsOrThrowsExactlyAsTheTableSays()
  {
    ...
    foreach (var outcome in Enum.GetValues<PrintOutcome>())
    {
      foreach (var bytesWritten in new[] { 0, 1, 512 })
      {
        var isDefinedByTheTable = bytesWritten > 0
                                    ? withBytes[outcome]
                                    : withoutBytes[outcome];

        if (isDefinedByTheTable)
        {
          Assert.That(_retryPolicy.Map(outcome, bytesWritten),
                      Is.Not.Null,
                      $"{outcome} with {bytesWritten} bytes");
        }
        else
        {
          Assert.Throws<InvalidOperationException>(() => _retryPolicy.Map(outcome, bytesWritten),
                                                   $"{outcome} with {bytesWritten} bytes");
        }
