...
public sealed class HandledOnPaperPolicyTest
{
  ...
  public void CanAcknowledge_AFailedUnknownOrBlockedTicket_IsAllowedUnderEveryStationCondition()
  {
    ...
    foreach (var ticketStatus in alwaysAllowed)
    {
      Assert.That(_policy.CanHandleOnPaper(ticketStatus, HealthyStation()), Is.True, $"{ticketStatus}");

      foreach (var station in StationsThatCannotPrint())
      {
  ...
  {
    Assert.That(_policy.CanHandleOnPaper(PrintJobStatus.Queued, HealthyStation() with { IsFaulty = true }),
                Is.True);
  }
  ...
  {
    Assert.That(_policy.CanHandleOnPaper(PrintJobStatus.Queued, HealthyStation() with { IsOnline = false }),
                Is.True);
  }
  ...
  {
    Assert.That(_policy.CanHandleOnPaper(PrintJobStatus.Queued, HealthyStation() with { IsPaperEnd = true }),
                Is.True);
  }
  ...
  {
    Assert.That(_policy.CanHandleOnPaper(PrintJobStatus.Queued, HealthyStation() with { IsCoverOpen = true }),
                Is.True);
  }
  ...
  {
    Assert.That(_policy.CanHandleOnPaper(PrintJobStatus.Queued, HealthyStation() with { IsInErrorState = true }),
                Is.True);
  }
  ...
  {
    Assert.That(_policy.CanHandleOnPaper(PrintJobStatus.Queued, HealthyStation() with { IsEnabled = false }),
                Is.True);
  }
