...
public sealed class HubGroupMembershipTest
{
  ...
  public async Task OrderAccepted_TwoStaffMembersConnected_ReachesOnlyThePlacingStaffMembersGroup()
  {
    ...
    await using var placingStaffMember = Connect(context.DeviceToken);
    await using var otherStaffMember = Connect(otherStaffMemberToken);
    ...
    var heard = await Task.WhenAny(placingStaffMemberHeard.Task, Task.Delay(patience));
    ...
    var silence = await Task.WhenAny(otherStaffMemberHeard.Task, Task.Delay(silenceWindow));
  ...
  {
    await using var connection = Connect(context.DeviceToken);
