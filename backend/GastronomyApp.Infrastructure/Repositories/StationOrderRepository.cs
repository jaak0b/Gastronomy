...
public sealed class StationOrderRepository : IStationOrderRepository
{
  ...
  public async Task<IReadOnlyList<QueuedStationOrder>> FindUnfinishedAtStationAsync(
    Guid festivalId,
    Guid stationId,
    CancellationToken cancellationToken)
  {
    IQueryable<StationOrder> stationOrders = StationOrdersAt(stationId)
      .Where(stationOrder => stationOrder.Items.Any(item => item.FulfilledAtUtc == null));

    return await BuildQueuedStationOrders(stationOrders, festivalId).ToListAsync(cancellationToken);
  }

  public async Task<IReadOnlyList<QueuedStationOrder>> FindFulfilledAtStationAsync(
    Guid festivalId,
    Guid stationId,
    CancellationToken cancellationToken)
  {
  ...
  public async Task<IReadOnlyList<OrderItem>> FindItemsAtStationAsync(IReadOnlyCollection<Guid> orderItemIds,
                                                                      Guid stationId,
                                                                      CancellationToken cancellationToken)
  {
    ...
    List<Guid> stationOrderIdsAtThisStation = await StationOrdersAt(stationId)
  ...
  public async Task<StationOrder?> FindAtStationAsync(Guid stationOrderId,
                                                      Guid stationId,
                                                      Guid festivalId,
                                                      CancellationToken cancellationToken)
  {
    return await _dbContext.StationOrders
                           .FirstOrDefaultAsync(stationOrder => stationOrder.Id == stationOrderId
                                                                && stationOrder.StationId == stationId
                                                                && stationOrder.FestivalId == festivalId,
                                                cancellationToken);
  }

  public async Task<IReadOnlyList<Guid>> FindOrderIdsOfStationOrdersAsync(
  ...
  public async Task<IReadOnlyList<OrderFulfillmentCounts>> FindFulfillmentCountsAsync(
    IReadOnlyCollection<Guid> orderIds,
    CancellationToken cancellationToken)
  {
    ArgumentNullException.ThrowIfNull(orderIds);

    List<Guid> ids = [.. orderIds];
    ...
    return await _dbContext.Orders
                           ...
                           .Select(order => new OrderFulfillmentCounts
                                            {
                                              OrderId = order.Id,
                                              ItemCount = order.StationOrders
                                                               .SelectMany(stationOrder => stationOrder.Items)
                                                               .Count(),
                                              FulfilledItemCount = order.StationOrders
                                                                        .SelectMany(stationOrder => stationOrder.Items)
                                                                        .Count(item => item.FulfilledAtUtc != null)
                                            })
                           .ToListAsync(cancellationToken);
  }

  public async Task<IReadOnlyList<StationQueuedWork>> FindQueuedWorkAtFestivalAsync(
    Guid festivalId,
    CancellationToken cancellationToken)
  {
    return await _dbContext.OrderItems
                                 ...
                                 (joined, catalogItem) => new StationQueuedWork
                                                          {
                                                            ...
                                                            Work = new QueuedWork(catalogItem.ProductionMinutes,
                                                                                  catalogItem.IsQueueIndependent)
                                                          })
                           .ToListAsync(cancellationToken);
  }
  ...
  {
    return _dbContext.StationOrders
                     .AsNoTracking()
                     .Where(stationOrder => stationOrder.StationId == stationId);
  }

  private IQueryable<QueuedStationOrder> BuildQueuedStationOrders(IQueryable<StationOrder> stationOrders,
                                                                  Guid festivalId)
  {
          ...
          .Select(joined => new QueuedStationOrder
                            {
                              ...
                              FulfilledItemCount = joined.StationOrder.Items
                                                                      .Count(item => item.FulfilledAtUtc != null),
                              Items = joined.StationOrder.Items
