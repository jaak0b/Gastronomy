...
public sealed class StaffMemberRepository : IStaffMemberRepository
{
  ...
  public async Task<IReadOnlyList<AdministeredStaffMember>> FindAdministeredAsync(DateTime nowUtc,
                                                                                  CancellationToken cancellationToken)
  {
    return await _dbContext.StaffMembers
                           ...
                           .Select(staffMember =>
                                     new AdministeredStaffMember(staffMember.Id,
                                                                 staffMember.Name,
                                                                 staffMember.IsActive,
                                                                 staffMember.DeviceId != null,
                                                                 _dbContext.Devices
                                                                           .Where(device => device.Id == staffMember.DeviceId)
                                                                           .Select(device => (DateTime?)device.LastSeenAtUtc)
                                                                           .FirstOrDefault(),
                                                                 _dbContext.EnrolmentInvitations
                                                                           .Any(invitation => invitation.Id == staffMember.EnrolmentInvitationId
  ...
  {
    return await _dbContext.StaffMembers
                           .FirstOrDefaultAsync(staffMember => staffMember.Id == staffMemberId, cancellationToken);
  }
