...
public sealed class NetworkPrinterSession : IPrinterSession
{
    ...
    public NetworkPrinterSession(TcpClient client, PrinterEndpoint endpoint, TimeProvider timeProvider)
    {
        this.client = client;
    ...
    public async Task<PrintDispatchResult> SendJobAsync(PrintPayload payload, CancellationToken cancellationToken)
    {
        ...
        lock (guard)
        {
            pendingEcho = new PendingProcessIdEcho(payload.ProcessId, echoCompletion);
        }
        ...
        try
        {
            ...
            await stream.WriteAsync(ProcessIdRequest(payload.ProcessId), cancellationToken);
        }
        ...
        Task completed = await Task.WhenAny(echoCompletion.Task, Task.Delay(endpoint.JobTimeout, cancellationToken));
        lock (guard)
        {
            pendingEcho = null;
        }
        ...
        if (completed != echoCompletion.Task)
        {
            return new PrintDispatchResult(
                ...
                bytesWritten,
                lastKnownStatus,
                $"No process id echo for {payload.ProcessId.ToString(CultureInfo.InvariantCulture)} arrived within {endpoint.JobTimeout}.");
        }
        ...
        return new PrintDispatchResult(
            ...
            bytesWritten,
            lastKnownStatus,
            $"Process id {payload.ProcessId.ToString(CultureInfo.InvariantCulture)} echoed on the sending connection.");
    }
    ...
    private PrintDispatchResult Dropped(int bytesWritten, string detail)
    {
        ...
        return new PrintDispatchResult(PrintAttemptOutcome.SocketDropped, bytesWritten, lastKnownStatus, detail);
    }
    ...
    private async Task<byte?> QueryAsync(int n, CancellationToken cancellationToken)
    {
        ...
        Task finished = await Task.WhenAny(completion.Task, Task.Delay(endpoint.StatusQueryTimeout, cancellationToken));
    ...
    private async Task HeartbeatLoopAsync(CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                await Task.Delay(endpoint.HeartbeatInterval, cancellationToken);
