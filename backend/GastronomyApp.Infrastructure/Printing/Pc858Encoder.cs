...
public sealed class Pc858Encoder
{
  ...
  {
    var printable = ToPrintableText(text);
    var bytes = new byte[printable.Length];
    for (var index = 0; index < printable.Length; index++)
    {
  ...
  private string Transliterate(char character)
  {
    ...
    {
      var isMark = CharUnicodeInfo.GetUnicodeCategory(part) == UnicodeCategory.NonSpacingMark;
