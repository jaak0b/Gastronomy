...
public sealed class StationQueueWriter
{
  ...
  public StationQueueWriter(IStationOrderRepository repository,
  ...
  public async Task<Result<FulfillmentResult, StationQueueFailure>> FulfillAsync(
    ...
    CancellationToken cancellationToken)
  {
    ...
    List<Guid> selectedIds = [.. orderItemIds];

    return await RunAsync(async transactionCancellationToken =>
                          {
                            IReadOnlyList<OrderItem> itemsAtThisStation =
                              await _repository.FindItemsAtStationAsync(selectedIds,
                                                                        stationId,
                                                                        transactionCancellationToken);

                            return _fulfillmentService.Fulfill(new() { OrderItemIds = selectedIds },
                                                               itemsAtThisStation,
                                                               _clock.UtcNow);
                          },
                          cancellationToken);
  ...
  public async Task<Result<FulfillmentResult, StationQueueFailure>> UnfulfillAsync(
    IReadOnlyCollection<Guid> orderItemIds,
    Guid stationId,
    CancellationToken cancellationToken)
  {
    ...
    return await RunAsync(async transactionCancellationToken =>
                          {
                            ...
                            return _fulfillmentService.Unfulfill(new() { OrderItemIds = selectedIds },
                                                                 itemsAtThisStation);
                          },
                          cancellationToken);
  }

  public async Task<Result<StationOrder, StationQueueFailure>> HideFromAsItComesQueueAsync(
    Guid stationOrderId,
    Guid stationId,
    Guid festivalId,
    CancellationToken cancellationToken)
  {
    StationOrder? stationOrder = await _repository.FindAtStationAsync(stationOrderId,
                                                                      stationId,
                                                                      festivalId,
                                                                      cancellationToken);
    ...
    {
      return Result<StationOrder, StationQueueFailure>.Failed(new()
                                                             {
                                                               Reason = StationQueueFailureReason.OrderNotAtThisStation
                                                             });
    }
    ...
    if (!hidden.IsSuccess)
    {
      return Result<StationOrder, StationQueueFailure>.Failed(new()
                                                             {
                                                               Reason = TranslateVisibilityReason(hidden.Failure.Reason)
                                                             });
    }
  ...
  private async Task<Result<FulfillmentResult, StationQueueFailure>> RunAsync(
    Func<CancellationToken, Task<Result<FulfillmentResult, FulfillmentFailure>>> body,
    CancellationToken cancellationToken)
  {
    Result<FulfillmentResult, FulfillmentFailure> outcome =
      await _transactionRunner.RunAsync(async transactionCancellationToken =>
                                        {
                                          Result<FulfillmentResult, FulfillmentFailure> applied =
                                            await body(transactionCancellationToken);
