...
public sealed class OrderStatusReader
{
  ...
  public async Task<IReadOnlyList<OrderStatusChange>> ReadStatusesOfStationOrdersAsync(IReadOnlyCollection<Guid> stationOrderIds, CancellationToken cancellationToken)
  {
    ...
    IReadOnlyList<OrderFulfillmentCounts> counts = await _repository.FindFulfillmentCountsAsync(orderIds, cancellationToken);

    return counts.Select(count => new OrderStatusChange
                                  {
