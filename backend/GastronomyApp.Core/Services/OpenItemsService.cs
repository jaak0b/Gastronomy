...
public sealed class OpenItemsService
{
  ...
  public OpenItemsService(IOpenItemRepository repository,
  ...
  public async Task<OpenItemsReport> ReadAsync(CancellationToken cancellationToken)
  {
    ...
    IReadOnlyList<OrderItem> openItems =
      await _repository.FindOpenAtFestivalAsync(festival.Id, cancellationToken);
    IReadOnlyList<OrderItem> givenAwayItems =
      await _repository.FindGivenAwayAtFestivalSinceAsync(festival.Id,
                                                          _clock.UtcNow - _givenAwayLookback,
                                                          cancellationToken);

    List<OrderItem> allItems = [.. openItems, .. givenAwayItems];
    IReadOnlyDictionary<Guid, OrderItemOwner> owners =
      await _repository.FindOwnersAsync([.. allItems.Select(item => item.Id).Distinct()], cancellationToken);
    ...
    return new()
           {
             ...
             [
               .. openByTable.Keys
                             ...
                             .Select(tableName => BuildOpenTable(tableName,
             ...
             OrderItemIdsWithoutAnOrder =
             [
               .. allItems.Where(item => !owners.ContainsKey(item.Id))
                          .Select(item => item.Id)
                          .Distinct()
             ]
           };
  ...
  public async Task<IReadOnlyList<string>> ReadTableNamesAsync(CancellationToken cancellationToken)
  {
    ...
    return festival is null
             ? []
             : await _repository.FindTableNamesAtFestivalAsync(festival.Id, cancellationToken);
  }
  ...
  private OpenTable BuildOpenTable(string tableName,
  ...
  {
    return items
          .Where(item => owners.ContainsKey(item.Id))
          .GroupBy(item => owners[item.Id].TableName, StringComparer.Ordinal)
  ...
  private IReadOnlyList<GivenAwayOrderItem> BuildGivenAwayItems(IEnumerable<OrderItem> items,
                                                                IReadOnlyDictionary<Guid, OrderItemOwner> owners)
  {
    return
    [
      .. items
        .Select(item => new GivenAwayOrderItem
                        {
                          OrderItemId = item.Id,
                          OrderId = owners[item.Id].OrderId,
                          GlobalOrderNumber = owners[item.Id].GlobalOrderNumber,
