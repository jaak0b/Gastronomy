...
{
  public Result<RoutingDecision, RoutingFailure> Resolve(Guid catalogItemId, IReadOnlyCollection<ItemStationAssignment> assignments, IReadOnlyCollection<Station> activeStations, Guid? chosenStationId)
  {
    ...
    if (chosenStationId is null)
    {
      ...
      return Result<RoutingDecision, RoutingFailure>.Success(new() { ResolvedStationId = candidates[0].Id });
    }
    ...
    {
      return Result<RoutingDecision, RoutingFailure>.Failed(new() { Reason = RoutingFailureReason.ChosenStationNoLongerPreparesTheItem });
    }

    return Result<RoutingDecision, RoutingFailure>.Success(new() { ResolvedStationId = chosenId });
  }
}
