...
public sealed class OrderQueryHandler
{
  ...
  public async Task<OrderDetailView> DescribeDetailAsync(LoadedOrder loaded, CancellationToken cancellationToken)
  {
    ...
    [
      .. loaded.StationOrders.SelectMany(stationOrder => lineCollapser
                                                        .Collapse([.. loaded.Items.Where(item => item.StationOrderId == stationOrder.Id)],
                                                                  item => item.ItemName,
                                                                  item => item.Note)
                                                        .Select(collapsed => new OrderDetailItemView(collapsed.Line.Id,
  ...
  private string StationNameFor(LoadedOrder loaded,
                                IReadOnlyDictionary<Guid, Guid> stationOrderStations,
                                Guid stationOrderId)
  {
    if (!stationOrderStations.TryGetValue(stationOrderId, out var stationId))
    {
      return string.Empty;
    }
