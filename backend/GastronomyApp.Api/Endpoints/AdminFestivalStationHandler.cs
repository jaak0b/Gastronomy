...
public sealed class AdminFestivalStationHandler
{
  ...
  public async Task<IResult> AddAsync(Guid festivalId, Guid stationId, CancellationToken cancellationToken)
  {
    Result<SavedFestivalStation, FestivalStationFailure> added =
      await _service.AddAsync(festivalId, stationId, cancellationToken);
  ...
  public async Task<IResult> RemoveAsync(Guid festivalId, Guid stationId, CancellationToken cancellationToken)
  {
    Result<SavedFestivalStation, FestivalStationFailure> removed =
      await _service.RemoveAsync(festivalId, stationId, cancellationToken);

    return await AnsweredAsync(removed, _ => Results.NoContent());
  }

  private async Task<IResult> AnsweredAsync(Result<SavedFestivalStation, FestivalStationFailure> written,
