...
public sealed class AdminStationHandler
{
  ...
  public AdminStationHandler(StationAdministrationService service,
  ...
  public async Task<IResult> ListAsync(Guid? festivalId, CancellationToken cancellationToken)
  {
    ...
    return Results.Ok(new AdminStationListView([.. listed.Value.Select(BuildStationView)]));
  }
  ...
  public async Task<IResult> CreateAsync(SaveStationRequest request, CancellationToken cancellationToken)
  {
    ...
    return await AnsweredAsync(created,
                               stationId => Results.Json(new SavedStationView(stationId),
                                         ...
                                         CancellationToken cancellationToken)
  {
    ...
    Result<SavedStation, StationAdministrationFailure> updated =
      await _service.UpdateAsync(stationId, BuildSaveRequest(request), cancellationToken);

    return await AnsweredAsync(updated,
                               savedStationId => Results.Ok(new SavedStationView(savedStationId)),
                               cancellationToken);
  }
  ...
  public async Task<IResult> ActivateAsync(Guid stationId, CancellationToken cancellationToken)
  {
    Result<SavedStation, StationAdministrationFailure> switchedOn =
      await _service.ActivateAsync(stationId, cancellationToken);

    return await AnsweredAsync(switchedOn,
                               savedStationId => Results.Ok(new SavedStationView(savedStationId)),
                               cancellationToken);
  }
  ...
  public async Task<IResult> DeactivateAsync(Guid stationId, CancellationToken cancellationToken)
  {
    Result<SavedStation, StationAdministrationFailure> switchedOff =
      await _service.DeactivateAsync(stationId, cancellationToken);

    return await AnsweredAsync(switchedOff,
                               savedStationId => Results.Ok(new SavedStationView(savedStationId)),
                               cancellationToken);
  }

  private SaveStationNameRequest BuildSaveRequest(SaveStationRequest request)
  {
    return new()
           {
             Name = request.Name,
  ...
  private AdminStationView BuildStationView(AdministeredStation station)
  {
    return new(station.StationId,
               ...
               station.HasOutstandingInvitation,
               station.IsAtTheFestival);
  }

  private async Task<IResult> AnsweredAsync(Result<SavedStation, StationAdministrationFailure> written,
