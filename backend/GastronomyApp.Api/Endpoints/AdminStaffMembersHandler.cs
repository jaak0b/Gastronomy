...
public sealed class AdminStaffMembersHandler
{
  ...
  public AdminStaffMembersHandler(StaffMemberAdministrationService service, DeviceRevocationAnnouncer revocationAnnouncer, ResultEnvelope resultEnvelope)
  {
    _service = service;
  ...
  public async Task<IResult> ListAsync(CancellationToken cancellationToken)
  {
    ...
    return Results.Ok(new AdminStaffMemberListView(staffMembers.Select(BuildStaffMemberView).ToList()));
  }
  ...
  public async Task<IResult> RenameAsync(Guid staffMemberId, RenameStaffMemberRequest request, CancellationToken cancellationToken)
  {
    ...
    Result<SavedStaffMember, StaffMemberAdministrationFailure> renamed = await _service.RenameAsync(staffMemberId, request.Name, cancellationToken);
  ...
  {
    Result<SavedStaffMember, StaffMemberAdministrationFailure> switchedOn = await _service.ActivateAsync(staffMemberId, cancellationToken);
  ...
  {
    Result<SavedStaffMember, StaffMemberAdministrationFailure> switchedOff = await _service.DeactivateAsync(staffMemberId, cancellationToken);
  ...
  {
    return new(staffMember.StaffMemberId, staffMember.Name, staffMember.IsActive, staffMember.HasDevice, staffMember.LastSeenAtUtc, staffMember.HasOutstandingInvitation);
  }

  private async Task<IResult> AnsweredAsync(Result<SavedStaffMember, StaffMemberAdministrationFailure> written, CancellationToken cancellationToken)
  {
