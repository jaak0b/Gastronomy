...
public sealed class AdminFestivalMenuHandler
{
  ...
  public async Task<IResult> PutOnTheMenuAsync(Guid festivalId,
                                               Guid itemId,
                                               ...
                                               CancellationToken cancellationToken)
  {
    ...
    Result<SavedFestivalMenuItem, FestivalMenuFailure> putOn =
      await _service.PutOnTheMenuAsync(festivalId,
                                       itemId,
                                       new()
                                       {
  ...
  public async Task<IResult> TakeOffTheMenuAsync(Guid festivalId, Guid itemId, CancellationToken cancellationToken)
  {
    Result<SavedFestivalMenuItem, FestivalMenuFailure> takenOff =
      await _service.TakeOffTheMenuAsync(festivalId, itemId, cancellationToken);

    return await AnsweredAsync(takenOff, festivalId, itemId, _ => Results.NoContent());
  }

  public async Task<IResult> SetAvailabilityAsync(Guid festivalId,
                                                  Guid itemId,
                                                  ...
                                                  CancellationToken cancellationToken)
  {
    ...
    Result<SavedFestivalMenuItem, FestivalMenuFailure> saved =
      await _service.SetAvailabilityAsync(festivalId, itemId, request.IsAvailable, cancellationToken);

    return await AnsweredAsync(saved, festivalId, itemId, savedItemId => Results.Ok(new SavedItemView(savedItemId)));
  }

  private async Task<IResult> AnsweredAsync(Result<SavedFestivalMenuItem, FestivalMenuFailure> written,
                                            Guid festivalId,
                                            Guid itemId,
    ...
    return failure.Reason switch
           {
             ...
             FestivalMenuFailureReason.PriceOutOfRange =>
               _resultEnvelope.Problem(StatusCodes.Status400BadRequest,
                                       "ValidationFailed",
                                       "admin.itemPriceOutOfRange"),
             FestivalMenuFailureReason.StationsDoNotBelongToTheFestival =>
               RefusedStationsOutsideTheFestival(failure.StationIdsOutsideTheFestival, festivalId, itemId),
             FestivalMenuFailureReason.NoStationPreparesTheItem =>
               _resultEnvelope.Problem(StatusCodes.Status422UnprocessableEntity,
                                       "UnprocessableEntity",
                                       "admin.itemNeedsAStation"),
             FestivalMenuFailureReason.FestivalIsRunning =>
  ...
  private IResult RefusedStationsOutsideTheFestival(IReadOnlyList<Guid> stationIdsOutsideTheFestival,
                                                    Guid festivalId,
                                                    Guid itemId)
  {
