...
public sealed class StationQueueHandler
{
  ...
  public StationQueueHandler(StationQueueService queueService,
  ...
  public async Task<IResult> ListQueueAsync(StationDeviceCaller caller, CancellationToken cancellationToken)
  {
    ...
    Result<StationQueue, StationQueueFailure> queue =
      await _queueService.ReadQueueAsync(caller.StationId, cancellationToken);
  ...
  public async Task<IResult> ListFulfilledAsync(StationDeviceCaller caller, CancellationToken cancellationToken)
  {
    ...
    Result<IReadOnlyList<QueuedStationOrder>, StationQueueFailure> stationOrders =
      await _queueService.ReadFulfilledAsync(caller.StationId, cancellationToken);
